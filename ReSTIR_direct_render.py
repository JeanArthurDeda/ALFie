...
def reservoir_update(r, p, n, f, w, l):
    r_p, r_n, r_w, r_s, r_m, r_l = r
    r_m += 1
    r_w += w
    r_s += f
    if random.random() * r_w < w:
        r_p = p
        r_n = n
        r_l = l
    return (r_p, r_n, r_w, r_s, r_m, r_l)

def reservoir_get_radiance(r):
    r_p, r_n, r_w, r_s, r_m, r_l = r
    return Vector((0, 0, 0)) if r_m == 0 else r_s / r_m

class ReSTIRVanillaEngine(bpy.types.RenderEngine):
    ...
    def restir (self, x, y, p, n, m, wo):
        ...
        def init_reservoir():
            ...
            for s in S:
                wi, pos, nor, pdf, cos_theta, l = s
                if cos_theta < 0.0: continue
                if not self.segment_visibility(p + n * SAMPLER_BIAS, pos): continue
                li = l.li
                f = bdrf(wi) * li * cos_theta / pdf
                w = f.length
                r = reservoir_update(r, pos, nor, f, w, l)
            return r
        
        def join_reservoir(r, pr, alpha):
            r_p, r_n, r_w, r_s, r_m, r_l = r
            pr_p, pr_n, pr_w, pr_s, pr_m, pr_l = pr
            if pr_p is None or pr_m == 0: return r

            wi = (pr_p - p).normalized()
            cos_theta = n.dot(wi)
            if cos_theta < 0: return r
            if not self.segment_visibility(p + n * SAMPLER_BIAS, pr_p): return r

            li = pr_l.li
            pdf, _ = pr_l.pdf(p, n, pr_p, pr_n)
            if pdf <= 0: return r
            f = bdrf(wi) * li * cos_theta / pdf
            w = min(f.length * alpha, self.ReSTIR_MAX_ENERGY)
            return reservoir_update(r, pr_p, pr_n, f, w, pr_l)
        
        r = init_reservoir()
        # r = join_reservoir(r, self.reservoir[y * self.size_x + x], self.ReSTIR_T_DECAY)
        ...
        #     r = join_reservoir(r, self.reservoir[ny * self.size_x + nx], 1.0)
        self.reservoir[y * self.size_x + x] = r
        return reservoir_get_radiance(r)
    
    def primary (self, x, y, p, wi):
        hit, pos, nor, mat = self.ray_trace(p, wi, SAMPLER_DISTANCE)
        if hit:
            l = self.restir(x, y, pos, nor, mat, -wi)
    ...
    def render(self, depsgraph):
        ...
        r = reservoir_new()
        self.reservoir = [r for _ in range(w*h)]
        ...
        for i in range(self.ReSTIR_H):
            for y in range(h):
                for x in range (w):
                    ...
                    image[(h-1-y) * w + x] = self.primary(x, y, cam_pos, d)
    ...
    def view_draw(self, context, depsgraph):
        ...
        if not self.draw_data or self.draw_data.dimensions != dimensions:
            self.draw_data = ReSTIRVanillaDrawData(dimensions)
...
def register():
    ...
    bpy.utils.register_class(ReSTIRVanillaEngine)

    for panel in get_panels():
        panel.COMPAT_ENGINES.add('MonteCarlo')

def unregister():
    bpy.utils.unregister_class(ReSTIRVanillaEngine)
