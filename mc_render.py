...
class AreaLight:
    ...
    def __init__(self, instance, ggx_mat_cache):
        ...
        obj = instance.object
        mesh = obj.data
        self.matrix_world = instance.matrix_world
        self.inv_matrix_world = instance.matrix_world.inverted()
        self.verts = [instance.matrix_world @ v.co for v in mesh.vertices]
        k_d, k_s, k_r, self.k_e, self.k_es = self.ggx_mat_cache.get(obj.material_slots[0].material)
        ...
        self.weight = self.area * self.k_es

        print (f"Area lignt {self.obj.name}")
        print (f"\tArea {self.area}")
        print (f"\tLi {self.k_es}")
...
class GenericightSampler(Sampler):
    ...
    def sample (self, p, n):
        ...
        if not hit or wi.dot(nor) >= 0.0:
            return wi, pdf, cos_theta, Vector((0, 0, 0))

        k_d, k_s, k_r, k_e, k_es = mat

        return wi, pdf, cos_theta, k_e * k_es
...
class AreaLightsImportanceSampler(Sampler):
    ...
    def sample(self, p, n):
        ...
        pos, nor, li = l.get_weight_random_point()
        ...
        if not is_visible(pos): li = Vector((0, 0, 0))
...
class MonterCarloRenderEngine(bpy.types.RenderEngine):
    # These three members are used by Blender to set up the
    # RenderEngine; define its internal name, visible name and capabilities.
    bl_idname = "MonteCarlo"
    bl_label = "MonteCarlo"
    ...
    def compute_randiance (self, p, n, m, wo, num):
        def bdrf(wi, n, wo, m):
        ...
        for _ in range(num):
            wi, pdf, cos_theta, li = self.sampler.sample(p, n)
            if pdf == 0.0: continue
    ...
    def primary (self, p, wi, num):
        ...
        if hit:
            l = self.compute_randiance(pos, nor, mat, -wi, num)
        ...
        for y in range(h):
            for x in range (w):
                ...
                if now - last_display > 4.0:
                    last_display = now
                    print (f"\tRender {int(100*((y * w + x) / (w*h)))} %")
        print (f"Rendering ({num}) done in {get_cosmetic_duration(perf_counter() - start)}")
    ...
    def view_draw(self, context, depsgraph):
        ...
        if not self.draw_data or self.draw_data.dimensions != dimensions:
            self.draw_data = MonteCarloDrawData(dimensions)
...
def register():
    ...
    bpy.utils.register_class(MonterCarloRenderEngine)
...
def unregister():
    bpy.utils.unregister_class(MonterCarloRenderEngine)
